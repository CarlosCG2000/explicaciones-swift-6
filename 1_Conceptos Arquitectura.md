# CONCEPTOS DE ARQUITECTURA: ÍNDICE
##  1_SWIFT BASADO EN `PROTOCOLOS`
## 2_Swift UI funciona con el concepto de `MVVM`
### ¿Qué es MVVM?
### ¿Puedo usar `@State` en lugar de `@StateObjec`t para vincular el `ViewModel`?
### ¿Por qué es importante `MVVM` en `SwiftUI`?
### ¿Cuándo usar `MVVM`?
### ¿Puedo tener varias `ViewModels` para varias `vistas` en `MVVM`?
### ¿Uso de `actores`, `clases` o `structs` en `lógica de negocio (Modelo)` o `lógica de presentación (ViewModel)`?
### DIFERENCIA ENTRE `UIKIT` CON `MVC` Y `SWIFT UI` CON `MVVM`.
## 3_LIBRERÍA DE `COMBINE` Y LA ACTUAL LIBRERÍA DE `Swift Macros`
### ¿Qué es `Boilerplate`?
## 4_`Clearn Architecture` que es un tipo de `arquitectura`
### `MVVM` se puede usar junto con `Clean Architecture`.
...
|     (Datos y          |       |                            |       |             |
|  logica de negocios)  |       |  (Lógica de presentación)  |       | (Interfaz)  |
+-----------------------+       +----------------------------+        +------------+
...
4. `Facilita las pruebas`:
 • Puedes probar la `lógica de negoci`o y el `estado en el ViewModel` sin necesidad de cargar la `interfaz de usuario`.
...
•	`Mejor manejo de estado`: Propiedades como `@State`, `@Binding` y `@Observable` facilitan el control del flujo de datos.
...
## `MVVM` se puede usar junto con `Clean Architecture`.
De hecho, `MVVM` puede ser una parte de la capa de `3. Interface Adapters` (esquema) dentro de `Clean Architecture`.
...
- Creamos un `protocolo` para `aislar` a un variable la URL, para que se pueda `inyectar un valor` distinto de URL y poder intercambiar de forma sencilla entre el json de `producción` y de `testing`.
...
## 10_@testable, Suite Testing, .tags (EN EL `TEST`)
-  En mi aplicación `StarWarsDemo` (`import StarWarsDemo`), pero añadimos el `@testable`, para que acceda a todos los componentes de nuestra aplicación y se pueda invocar dichos componentes.
